  ...
  const carregarInventarios = async () => {
    try {
      ...
      for (let inv of data.inventarios || []) {
        try {
          const salasResponse = await fetch(`/api/salas?inventario=${inv.nome}`);
          const salasData = await salasResponse.json();
          const quantidadeSalas = Array.isArray(salasData) ? salasData.length : 0;
          
          const itensResponse = await fetch(`/api/inventario?inventario=${inv.nome}`);
          const itensData = await itensResponse.json();
          const quantidadeItens = Array.isArray(itensData) ? itensData.length : 0;
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg">
        ...
        <p>Esta página mostra todos os inventários disponíveis e qual contém dados.</p>
        <p className="text-sm text-gray-600">
          Usuário: {session?.user?.email} | Total: {inventarioDetalhes.length} inventários
        </p>
      </div>
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="text-right">
                <div className="text-sm">
                  📁 {inv.quantidadeSalas} salas | 📋 {inv.quantidadeItens} itens
                </div>
            ...
            <div className="flex gap-2 mb-2">
              <span className={`px-2 py-1 rounded text-xs ${
                inv.temDados ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
              }`}>
